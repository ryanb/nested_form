...
By default <tt>fields_for</tt> inside <tt>nested_form_for</tt> will add <div class="fields"> wrapper for every nested object. Sometimes you need not the <div>, but the <tr> (if you insert rows into table). Now you can specify wrapping tag by providing <tt>:wrapper_tag => 'tr'</tt> option (by default it is 'div'):
